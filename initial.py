...
if __name__ == "__main__":
  if active and '2' in run_stage:
    ...
    total = len(records)
    if total == 0:
      cur.execute("UPDATE images_alignment SET alignment_stage = 2 FROM (SELECT id FROM images_alignment WHERE alignment_stage = 2002 ORDER BY id LIMIT 2) s WHERE s.id = images_alignment.id")
      cur.connection.commit()
      gc.collect()
    count = 0
    print records
