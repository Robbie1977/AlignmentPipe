...
class Mask_original(models.Model):
    image = models.ForeignKey(Original_nrrd, limit_choices_to={'alignment__alignment_stage: 0'})
...
class Mask_aligned(models.Model):
    ...
    def mask_image(self):
      # if self.complete:
      #   return '<img src="/images/nrrd/mask_aligned_%s/%s"/>' % str(self.channel), str(self.id)
      return '<img src="/static/waiting.gif"/>'
    mask_image.short_description = 'detected objects'
    mask_image.allow_tags = True
    def orig_image(self):
