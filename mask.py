  ...
  if active and '0' in run_stage:
    ...
    for line in records:
      ...
      if line[6]:
        ...
        cur.execute("INSERT INTO images_alignment ( name, settings, max_stage, last_host, aligned_score, alignment_stage, orig_orientation, loading_host, original_ext, original_path, crop_xyz, temp_initial_nrrd, temp_initial_score, background_channel, signal_channel, ac1_channel, aligned_bg, aligned_sg, aligned_ac1, aligned_slice_score, aligned_avgslice_score, notes, reference, user, id ) SELECT %s, settings, max_stage, last_host, aligned_score, alignment_stage, orig_orientation, loading_host, original_ext, original_path, crop_xyz, temp_initial_nrrd, temp_initial_score, background_channel, signal_channel, ac1_channel, aligned_bg, aligned_sg, aligned_ac1, aligned_slice_score, aligned_avgslice_score, notes, reference, user, NULL FROM images_alignment WHERE image = %s", [newName, oldId])
        cur.connection.commit()
        gc.collect()
